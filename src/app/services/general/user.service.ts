...
export class UserService {
  ...
  createUser(user: UserData): Observable<UserData> {
    return this.http
      .post<UserData>(`${environment.fireBaseDBurl}/users.json`, user)
      .pipe(
  ...
  createLogin(userId: string, login: LoginOffice): Observable<LoginOffice> {
    return this.http.put<LoginOffice>(
      `${environment.fireBaseDBurl}/users/${userId}/login.json`,
      login
    );
  }
  ...
  createSessionData(
    ...
    return this.http.put<SessionInterface>(
      `${environment.fireBaseDBurl}/users/${userId}/session.json`,
      session
    );
  }
  ...
  getAll(): Observable<UserData[]> {
    return this.http.get(`${environment.fireBaseDBurl}/users.json`).pipe(
  ...
  getUserById(id: string): Observable<UserData> {
    return this.http
      .get<UserData>(`${environment.fireBaseDBurl}/users/${id}.json`)
      .pipe(
        map((user: UserData) => {
          return {
            ...user,
  ...
  getLoginById(id: string): Observable<LoginOffice> {
    return this.http.get<LoginOffice>(
      `${environment.fireBaseDBurl}/users/${id}/login.json`
    );
  }

  getSessionById(id: string): Observable<SessionInterface> {
    return this.http.get<SessionInterface>(
      `${environment.fireBaseDBurl}/users/${id}/session.json`
    );
  }

  // Отримати глобальний лічильник
  getCounts(): Observable<CountsActive> {
    return this.http.get<CountsActive>(
      `${environment.fireBaseDBurl}/counts.json`
    );
  }

  getByPhone(phone: number): Observable<UserData | null> {
    return this.http
      .get<{ [key: number]: UserData }>(
  ...
  remove(id: string): Observable<void> {
    return this.http.delete<void>(
      `${environment.fireBaseDBurl}/users/${id}.json`
    );
  }
}
